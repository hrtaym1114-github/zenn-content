...
この制約を突破するのが **cmux** です。ただし2026年2月現在、「cmux」という名前のツールが **2つ** あり、それぞれ **CLIツール（Worktree管理）** と **GUIターミナルアプリ（視覚的セッション管理）** という全く異なるカテゴリのツールです。筆者は両方を実際に使い分けており、本記事ではその実体験をもとに以下を解説します。

- 2つのcmuxの **カテゴリの違い** と、コマンド名衝突の注意点
...
なお、このアプリは元々「**cmuxterm**」という名前で開発されていましたが、2026年2月9日のv0.23.0で「cmux」にリネームされました（[Changelog](https://www.cmux.dev/docs/changelog)）。この経緯が、同名ツールが2つ存在する原因です。

### そもそもカテゴリが違う

この2つは名前こそ同じですが、**全く異なるカテゴリ**のツールです。

| 観点 | cmux CLI（craigsc） | cmux App（manaflow-ai） |
|------|-------------------|----------------------|
| **カテゴリ** | CLIユーティリティ（Bashスクリプト） | **ターミナルエミュレータ**（GUIアプリ） |
| **何をするか** | Worktreeのライフサイクル管理 | 複数セッションの視覚的管理・通知 |
| **tmuxとの関係** | tmuxやターミナルの**中で**動く | tmuxやターミナルの**代わり** |
| **OS** | macOS / Linux | macOSのみ |

つまり、cmux CLIは「Worktreeを素早く作ってClaude Codeを並列起動するランチャー」、cmux Appは「複数のAIエージェントを1つのウインドウで視覚的に管理するターミナル」です。

### コマンド名の衝突に注意

:::message alert
**両方のCLIコマンド名が `cmux` で同じです。** craigsc版はシェル関数（`source`で読み込み）、manaflow-ai版はバイナリ（`/usr/local/bin/cmux`）として配置されます。両方インストールした場合、ターミナルで `cmux` と打つとシェル関数（craigsc版）が優先されます。

さらに、manaflow-ai版のターミナル内ではそちらのCLIが自動登録されるため、cmux App内でcraigsc版のコマンドを実行しようとすると衝突する可能性があります。
:::

**実用上の対処**: cmux Appを**GUIアプリ**（Dock / Spotlight）から起動し、cmux CLIを**通常のターミナル**（iTerm2、標準ターミナル等）で使えば衝突は発生しません。筆者の環境でも実際にこの組み合わせで問題なく動作しています。

manaflow-ai版のCLIもターミナルから使いたい場合は、フルパスで実行するか、エイリアスを設定します:

```bash
# 例: manaflow-ai版CLIを別名で使う
alias cmux-app="/Applications/cmux.app/Contents/Resources/bin/cmux"
```
...
まず自分のスタイルに合ったツールを選びます:

- **GUIで視覚管理したい / macOS** → cmux App（manaflow-ai版）
- **CLIでWorktreeを管理したい / Linux対応** → cmux CLI（craigsc版）
- **両方使う場合** → GUIアプリ + 別ターミナルでCLI（[衝突の注意点](#コマンド名の衝突に注意)を確認）

### セットアップ

- [ ] 選んだツールをインストール
- [ ] cmux CLI利用時: `.gitignore`に`.worktrees/`を追加
- [ ] cmux CLI利用時: `.cmux/setup`フック作成（`cmux init`で自動生成可）
- [ ] CLAUDE.md整備（2,500トークン以下）

### 日常ワークフロー（cmux CLI利用時）
...
cmux CLI / cmux Appはこの並列運用を**最小限のオーバーヘッド**で実現するツールです。自分のスタイルに合った方を選んで、まずは1つのWorktreeから始めてみてください。
